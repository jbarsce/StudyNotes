...
\underline{TL;DR}
DDPG \cite{lillicrap_continuous_2015} is an off-policy actor critic algorithm that trains simultaneously a deterministic actor function $a$ and a critic function $Q$ with an experience memory.
...
SAC \cite{haarnoja_soft_2018} is an off-policy actor critic algorithm based in maximum entropy reinforcement learning with an stochastic actor (i.e. an actor that learns from sampling actions based on the current policy instead of using past actions from memory) with the aim of producing more stable and efficient learning.

\underline{Problem addressed}

Using an off-policy algorithm, how to achieve stability in learning?
...
\begin{itemize}
  \item Learning $Q_{\phi_1}$ and $Q_{\phi_2}$ functions as in TD3, a policy $\pi_\theta$ and a value function $V_\psi$.
  \item Using entropy regularization ...
\end{itemize}
...
\begin{itemize}
  \item A2C \url{https://arxiv.org/pdf/1602.01783.pdf}
